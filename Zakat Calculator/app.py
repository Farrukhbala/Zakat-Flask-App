...
class CalculateZakat(Form):
  gold = StringField('Gold' ,[validators.length(min=2 , max=30)])
  silver = StringField('Silver' ,[ validators.length(min=2 , max=30)])
  cash = StringField('Cash' , [validators.length(min=5 , max= 30)])
  properties = StringField('Property' ,[ validators.length(min=2 , max=30)])
  liabilities = StringField('Liabilities' , [validators.length(min=5 , max= 30)])
